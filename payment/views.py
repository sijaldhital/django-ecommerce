...
def process_order(request):
    if request.POST:
        ...
        if request.user.is_authenticated:
            ...
            # Delete our Cart from DB for only authenticated users (old_cart field)
            current_user = Profile.objects.filter(user__id=request.user.id)
            # Now lets delete the old cart
...
def billing_info(request):
    if request.POST:
          ...
          paypal_dict = {
              ...
              'amount': totals,
              'item_name': 'Tech Order',
              'no_shipping': '2',
              'invoice': str(uuid.uuid4()),
              'currency_code': 'USD',
              'notify_url': 'https://{}{}'.format(host, '/payment/paypal/'),
